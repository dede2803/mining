...
function curl($url, $post = 0, $httpheader = 0, $proxy = 0){ 
    ...
    curl_setopt($ch, CURLOPT_COOKIEFILE,"cookieDogeYUT.txt");
    curl_setopt($ch, CURLOPT_COOKIEJAR,"cookieDogeYUT.txt");
...
for ($adaPayuTop = 0; $adaPayuTop < 50; $adaPayuTop++) 
{
    ...
    if($IP != ""){
        ...
        */
        ...
        function get(){
            $url = "https://dogepayu.top";
            return curl($url,'',head())[1];
        }
            
        function login($email,$token,$header){
            $url="https://dogepayu.top/auth/login";
        ...
        function test() {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL,"https://dogepayu.top/faucet/currency/doge");
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        ...
        function currency(){
            $url = "https://dogepayu.top/faucet/currency/doge";
            return curl($url,'',head())[1];
        }
            
        function claim($aft,$ctn,$tkn,$header){
            $url = "https://dogepayu.top/faucet/verify/doge";
        ...
        $login=login($email,$token,array_merge(head(),array("Referer: https://dogepayu.top/")));
        sleep(5);
        ...
        while(true)
        {
            for ($adaPayuTop = 0; $adaPayuTop < 50; $adaPayuTop++) {
                ...
                if($sttsBal == "Empty"){
                    ...
                    sleep(2);
                    system('rm cookieDogeYUT.txt');
                    sleep(2);
                    system('php feyYUT.php');
                }
                ...
                if($alert == "danger"){
                    echo $WhiteTebal. "[1]. Server ". $YellowTebal . "PAYU TOP ". $WhiteTebal. "Coin Crypto ". $YellowTebal . "DOGE ". $RedTebal. "Habis.".$WhiteTebal. "Tunggu Besok Lagi !!!\n";
                    sleep(2);
                    session_destroy(true);
                    sleep(2);
                    system('rm cookieDogeYUT.txt');
                    ...
                    if($aft == ""){
                        ...
                        wktClaim($timeClaim);
                        sleep(2);
                        $claim = claim($aft,$ctn,$tkn,array_merge(head(),array("Referer: https://dogepayu.top/faucet/currency/doge")));
                    
                        echo $WhiteTebal ."Selamat, claim ".$GreenTebal."BERHASIL.!!!";
                        sleep(2);
                        echo "\r                                                                \r";
                        sleep(2);
                        echo $WhiteTebal ."SERVER        = ". $YellowTebal. "DOGE PAYU TOP\n";
                        sleep(2);
