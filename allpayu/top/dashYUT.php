...
function curl($url, $post = 0, $httpheader = 0, $proxy = 0){ 
    ...
    curl_setopt($ch, CURLOPT_COOKIEFILE,"cookieDashYUT.txt");
    curl_setopt($ch, CURLOPT_COOKIEJAR,"cookieDashYUT.txt");
...
for ($adaPayuTop = 0; $adaPayuTop < 50; $adaPayuTop++) 
{
    ...
    if($IP != ""){
        ...
        */
        ...
        function get(){
            $url = "https://dashpayu.top";
            return curl($url,'',head())[1];
        }
            
        function login($email,$token,$header){
            $url="https://dashpayu.top/auth/login";
        ...
        function test() {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL,"https://dashpayu.top/faucet/currency/dash");
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        ...
        function currency(){
            $url = "https://dashpayu.top/faucet/currency/dash";
            return curl($url,'',head())[1];
        }
            
        function claim($aft,$ctn,$tkn,$header){
            $url = "https://dashpayu.top/faucet/verify/dash";
        ...
        $login=login($email,$token,array_merge(head(),array("Referer: https://dashpayu.top/")));
        sleep(5);
        ...
        while(true)
        {
            for ($adaPayuTop = 0; $adaPayuTop < 50; $adaPayuTop++) {
                ...
                if($sttsBal == "Empty"){
                    echo $check2;
                    echo $WhiteTebal. "Server ". $YellowTebal . "PAYU TOP ". $WhiteTebal. "Coin Crypto ". $YellowTebal . "DASH ". $RedTebal. "Sedang Kosong.".$WhiteTebal. "Tunggu Besok Lagi !!!\n";
                    sleep(2);
                    session_destroy(true);
                    sleep(2);
                    system('rm cookieDashYUT.txt');
                    sleep(2);
                    system('php dgbYUT.php');
                }
                ...
                if($alert == "danger"){
                    echo $WhiteTebal. "[1]. Server ". $YellowTebal . "PAYU TOP ". $WhiteTebal. "Coin Crypto ". $YellowTebal . "DASH ". $RedTebal. "Habis.".$WhiteTebal. "Tunggu Besok Lagi !!!\n";
                    sleep(2);
                    session_destroy(true);
                    sleep(2);
                    system('rm cookieDashYUT.txt');
                    ...
                    if($aft == ""){
                        ...
                        wktClaim($timeClaim);
                        sleep(2);
                        $claim = claim($aft,$ctn,$tkn,array_merge(head(),array("Referer: https://dashpayu.top/faucet/currency/dash")));
                    
                        echo $WhiteTebal ."Selamat, claim ".$GreenTebal."BERHASIL.!!!";
                        sleep(2);
                        echo "\r                                                                \r";
                        sleep(2);
                        echo $WhiteTebal ."SERVER        = ". $YellowTebal. "DASH PAYU TOP\n";
                        sleep(2);
